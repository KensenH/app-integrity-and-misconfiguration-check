...
func NewCmdScan() *cobra.Command {
	...
	cmd.PersistentFlags().BoolVar(&debug, "debug", false, "turn on debug logs")
	cmd.PersistentFlags().BoolVar(&absolutePath, "absolute-path", false, "use the absolute path for the file name")
	cmd.PersistentFlags().StringVarP(&format, "format", "f", "json", "Set output format (json, template)")
	cmd.PersistentFlags().StringVar(&schemaDir, "schema-dir", "", "Sets the directory for the json schemas")
	cmd.Flags().StringVarP(&template, "template", "t", "", "Set output template, it will check for a file or read input as the")
	cmd.PersistentFlags().StringVarP(&outputLocation, "output", "o", "", "Set output location")
	cmd.PersistentFlags().IntVar(&exitCode, "exit-code", 2, "Set the exit-code to use on failure")
